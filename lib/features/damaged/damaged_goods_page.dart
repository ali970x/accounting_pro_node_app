...
class _DamagedGoodsPageState extends State<DamagedGoodsPage> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      _movements = (results[1] as List).map((e) => _DamageMovement.fromJson(Map<String, dynamic>.from(e as Map))).toList();
    } catch (e) {
      _error = e.toString();
    }
  ...
  Widget build(BuildContext context) {
    ...
    final damagedQty = _movements.fold<double>(0, (sum, row) => sum + row.quantity);
      ...
      child: ListView(
        ...
        children: [
            ...
            children: [
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(isAr ? "البضاعة التالفة" : "Damaged Goods", style: theme.textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.w900)),
                    Text(
                      isAr ? "انقل من المخزون واحسب خسارة التالف مباشرة" : "Move stock into damage and track the loss",
          ...
          if (_loading)
            ...
            ModernCard(child: Text(_error!, style: const TextStyle(color: Colors.red)))
          else ...[
  ...
  Widget _summaryGrid(bool isAr, Map<String, double> totals, double damagedQty) {
        ...
        children: [
          ...
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(value, style: theme.textTheme.titleMedium?.copyWith(fontWeight: FontWeight.w900, color: color, height: 1.25)),
  ...
  Widget _movementList(bool isAr) {
    if (_movements.isEmpty) {
      return ModernCard(
        child: Padding(
          padding: const EdgeInsets.all(18),
          child: Text(isAr ? "لا يوجد بضاعة تالفة مسجلة بعد." : "No damaged goods recorded yet."),
        ),
      );
    }
    ...
    return ModernCard(
      ...
      child: ExpansionTile(
        ...
        title: Text(isAr ? "سجل التالف" : "Damage history", style: const TextStyle(fontWeight: FontWeight.w900)),
        subtitle: Text(isAr ? "${_movements.length} حركة" : "${_movements.length} movements"),
        childrenPadding: const EdgeInsets.fromLTRB(12, 0, 12, 14),
        children: [
          ..._movements.take(25).map((row) => Padding(
  ...
  void _clearActiveIfHidden() {
    ...
    final visible = _filteredChoices().any((choice) => choice.id == selected.id);
    if (!visible) {
      _activeChoice = null;
      _productSearch.clear();
      _unitCost.clear();
